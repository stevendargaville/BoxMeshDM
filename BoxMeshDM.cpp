...
static void ResolveBoundaryOwnership(MPI_Comm comm, std::vector<Point> &points, double min_x, double min_y, double max_x,
                                     double max_y, double interior_min_x, double interior_min_y, double interior_max_x,
                                     double interior_max_y, double pad) {
    int rank, size;
            ...
            for (int dy = -1; dy <= 1; ++dy) {
                for (int dx = -1; dx <= 1; ++dx) {
                    ...
                    if (nx >= 0 && nx < TILE_DIM_X && ny >= 0 && ny < TILE_DIM_Y) {
                        ...
                        bool relevant = true;
                        if (p.x < n_min_x - safe_pad)
                            relevant = false;
                        if (p.x > n_max_x + safe_pad)
                            relevant = false;
                        if (p.y < n_min_y - safe_pad)
                            relevant = false;
                        if (p.y > n_max_y + safe_pad)
                            relevant = false;
...
static double next_double(RngState &state) { return (splitmix64(state.s) >> 11) * (1.0 / 9007199254740992.0); }
...
static Point create_point_with_unique_hash_id(double x, double y, int ix, int iy, int type) {
    ...
    uint64_t id = ((uint64_t)(type & 0x3) << 62) | ((uint64_t)(ix & 0x7FFFFFFF) << 31) | ((uint64_t)(iy & 0x7FFFFFFF));
    return {x, y, id};
}
...
static void apply_jitter(std::vector<Point> &points, double amount, int seed_offset) {
    for (size_t i = 0; i < points.size(); ++i) {
        ...
        if (!was_boundary) {
            ...
            if (points[i].x < 0)
                points[i].x = 0;
            if (points[i].x > DOMAIN_WIDTH)
                points[i].x = DOMAIN_WIDTH;
            if (points[i].y < 0)
                points[i].y = 0;
            if (points[i].y > DOMAIN_HEIGHT)
                points[i].y = DOMAIN_HEIGHT;
        }
    }
}
...
static double get_max_cosine_tri(const Point &a, const Point &b, const Point &c) {
    ...
    if (ab < TOL_LEN || bc < TOL_LEN || ca < TOL_LEN)
...
static void relax_points_lloyd(std::vector<Point> &points, const std::vector<Triangle> &triangles, double min_safe_x,
                               double min_safe_y, double max_safe_x, double max_safe_y) {
    int n = points.size();
        ...
        if (points[i].x > min_safe_x && points[i].x < max_safe_x && points[i].y > min_safe_y && points[i].y < max_safe_y) {
            ...
            for (double factor : candidate_factors) {
                ...
                for (int j = start; j < end; ++j) {
                    ...
                    if (mc > candidate_max_cos)
                        candidate_max_cos = mc;
                }
...
static void relax_points_spring(std::vector<Point> &points, const std::vector<Triangle> &triangles, double min_safe_x,
                                double min_safe_y, double max_safe_x, double max_safe_y) {
    int n = points.size();
...
static void ComputeValenceAndEdges(const std::vector<Point> &points, const std::vector<Triangle> &triangles,
                                   std::vector<int> &valence, std::vector<std::pair<int, int>> &unique_edges) {
...
static void process_tile(MPI_Comm comm, int final_smooth_its, int tile_x, int tile_y,
                         std::vector<Point> &points_on_owned_triangles_and_orphans, std::vector<Triangle> &triangles_owned) {
    ...
    double tile_s_x = DOMAIN_WIDTH / TILE_DIM_X;
    double tile_s_y = DOMAIN_HEIGHT / TILE_DIM_Y;
    double t_min_x = tile_x * tile_s_x;
    double t_max_x = (tile_x + 1) * tile_s_x;
    double t_min_y = tile_y * tile_s_y;
    double t_max_y = (tile_y + 1) * tile_s_y;
    ...
    if (pad > min_dim / 2.0) {
        std::cerr << "Error: Annealing iters (" << ANNEAL_ITERS << ") require a halo of " << pad
                  << ", which exceeds the tile size (" << min_dim << ").\n"
                  << "Reduce ANNEAL_ITERS or increase tile size (by having fewer tiles or more points per tile).\n";
        std::exit(EXIT_FAILURE);
    }

    double search_min_x = t_min_x - pad;
    double search_max_x = t_max_x + pad;
    double search_min_y = t_min_y - pad;
    double search_max_y = t_max_y + pad;

    double interior_min_x = t_min_x + pad;
    double interior_max_x = t_max_x - pad;
    double interior_min_y = t_min_y + pad;
    double interior_max_y = t_max_y - pad;
    ...
    // (0,0)
    if (search_min_x <= EPSILON && search_max_x >= -EPSILON && search_min_y <= EPSILON && search_max_y >= -EPSILON) {
        points_with_halos.push_back(create_point_with_unique_hash_id(0.0, 0.0, 0, 0, 1));
    }
    // (1,0)
    if (search_min_x <= DOMAIN_WIDTH + EPSILON && search_max_x >= DOMAIN_WIDTH - EPSILON && search_min_y <= EPSILON &&
        search_max_y >= -EPSILON) {
        points_with_halos.push_back(create_point_with_unique_hash_id(DOMAIN_WIDTH, 0.0, max_idx, 0, 1));
    }
    // (0,1)
    if (search_min_x <= EPSILON && search_max_x >= -EPSILON && search_min_y <= DOMAIN_HEIGHT + EPSILON &&
        search_max_y >= DOMAIN_HEIGHT - EPSILON) {
    ...
    for (int iy = min_iy; iy <= max_iy; ++iy) {
        for (int ix = min_ix; ix <= max_ix; ++ix) {
            ...
            if (cx < exclusion)
                continue;
            if (cx > DOMAIN_WIDTH - exclusion)
                continue;
            if (cy < exclusion)
                continue;
            if (cy > DOMAIN_HEIGHT - exclusion)
                continue;
            ...
            // else
            // {
            //    std::cerr << "Warning: Hash collision detected for point (" << cx << ", " << cy << ") ID: " <<
    ...
    for (int iter = 0; iter < ANNEAL_ITERS; ++iter) {
        ...
        ResolveBoundaryOwnership(comm, points_with_halos, s_min_x, s_min_y, s_max_x, s_max_y, interior_min_x, interior_min_y,
                                 interior_max_x, interior_max_y, pad);
    }
...
static DM CreateDM(MPI_Comm comm, const std::vector<Point> &points_on_owned_triangles_and_orphans,
    ...
    PetscInt two = 2;
    PetscInt three = 3;
    ierr = DMPlexCreateFromCellListParallelPetsc(comm, two, num_tris_owned, num_points_owned, PETSC_DECIDE, three, PETSC_TRUE,
                                                 cells.data(), two, coords_points_owned.data(), NULL, NULL, &dm);
...
static void LabelBoundaries(DM dm) {
    ...
    for (PetscInt v = vStart; v < vEnd; ++v) {
        ...
        if (dof > 0) {
            ...
            if (val != 0)
                PetscCallVoid(DMLabelSetValue(label, v, val));
        }
    }
...
static bool CheckMeshIntegrity(MPI_Comm comm, const std::vector<Point> &points_on_owned_triangles_and_orphans,
                               const std::vector<Triangle> &triangles_owned, std::vector<int> &valence,
    ...
    for (size_t i = 0; i < points_on_owned_triangles_and_orphans.size(); ++i) {
        ...
        if (valence[i] == 0 && appears_in_triangle[i] && get_owner_rank(p, size) == rank) {
            std::cout << "[Rank " << rank << "] UNCONNECTED POINT: (" << p.x << ", " << p.y << ") ID: " << p.unique_hash_id
                      << " Valence: " << valence[i] << "\n";
        }
    }
    ...
    for (const auto &t : triangles_owned) {
        ...
        if (d01 > THRESHOLD_LEN || d12 > THRESHOLD_LEN || d20 > THRESHOLD_LEN) {
            ...
            if (bad_edge_print_count < MAX_BAD_PRINTS) {
                std::cout << "[Rank " << rank << "] BAD TRIANGLE: Edge len " << std::max({d01, d12, d20}) << " vs target "
                          << TARGET_EDGE_LENGTH << "\n";
                std::cout << "   P0: (" << p0.x << ", " << p0.y << ") Owner: " << get_owner_rank(p0, size)
                          << " ID: " << p0.unique_hash_id << "\n";
                std::cout << "   P1: (" << p1.x << ", " << p1.y << ") Owner: " << get_owner_rank(p1, size)
                          << " ID: " << p1.unique_hash_id << "\n";
                std::cout << "   P2: (" << p2.x << ", " << p2.y << ") Owner: " << get_owner_rank(p2, size)
                          << " ID: " << p2.unique_hash_id << "\n";
        ...
        for (int i = 0; i < 3; i++) {
            ...
            bool is_bdy = false;
            if (std::abs(ep1.x) < EPSILON && std::abs(ep2.x) < EPSILON)
                is_bdy = true;
            else if (std::abs(ep1.x - DOMAIN_WIDTH) < EPSILON && std::abs(ep2.x - DOMAIN_WIDTH) < EPSILON)
                is_bdy = true;
            else if (std::abs(ep1.y) < EPSILON && std::abs(ep2.y) < EPSILON)
                is_bdy = true;
            else if (std::abs(ep1.y - DOMAIN_HEIGHT) < EPSILON && std::abs(ep2.y - DOMAIN_HEIGHT) < EPSILON)
                is_bdy = true;
    ...
    if (rank == 0) {
        ...
        if (!area_pass || !perim_pass || !euler_pass || !edge_pass) {
            ...
            if (!area_pass)
                std::cout << "  [FAIL] Total Area: " << std::fixed << std::setprecision(6) << global_total_area
                          << " (Expected 1.0)\n";
            if (!perim_pass)
                std::cout << "  [FAIL] Boundary Perimeter: " << global_boundary_len << " (Expected 4.0)\n";
            if (!euler_pass)
...
static void ComputeAndPrintStats(MPI_Comm comm, int final_smooth_its,
                                 ...
                                 const std::vector<Triangle> &triangles_owned, const std::vector<int> &valence,
    ...
    for (const auto &t : triangles_owned) {
        ...
        // volume
        double volume = 0.5 * std::abs((p1.x - p0.x) * (p2.y - p0.y) - (p1.y - p0.y) * (p2.x - p0.x));
        if (volume < local_min_volume)
            local_min_volume = volume;
    ...
    if (rank == 0) {
        ...
        std::cout << "  Imbalance Ratio (Max/Avg): "
                  << (double)max_points_owned_global / ((double)num_points_owned_global / size) << "\n";
...
PETSC_EXTERN DM GenerateBoxMeshDM(MPI_Comm comm, double target_edge_length, double domain_width, double domain_height,
                                  int final_smooth_its, PetscBool integrity_check, PetscBool print_stats) {
    int comm_rank, comm_size;
    MPI_Comm_rank(comm, &comm_rank);
    MPI_Comm_size(comm, &comm_size);
    ...
    // 4. Print stats
    if (print_stats)
        ComputeAndPrintStats(comm, final_smooth_its, points_on_owned_triangles_and_orphans, triangles_owned, valence,
                             unique_edges);
...
int main(int argc, char **argv) {
    ...
    DM dm = GenerateBoxMeshDM(MPI_COMM_WORLD, target_len, domain_width, domain_height, final_smooths, integrity_check,
                              print_stats);

    // Check a valid mesh has been generated
    if (dm) {
        // Write output if requested
        WriteTetrahedralMesh(dm, "box_mesh", MPI_COMM_WORLD, write_mesh, print_stats);
